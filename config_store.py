...
class ConfigStore:
    ...
    def get_profile(self, profile_id=None):
        ...
        return next((p for p in self.profiles() if p.get("id") == pid), None)

    def get_dsn(self, profile_id=None):
        p = self.get_profile(profile_id)
        if not p:
            return None
        try:
            return keyring.get_password(SERVICE, f"dsn:{p['id']}")
        except Exception:
            return None
    ...
    def get_plan(self, plan_id=None):
        ...
        return next((p for p in self.plans() if p.get("id") == pid), None)
    ...
    def delete_plan(self, plan_id):
        self.data["plans"] = [p for p in self.plans() if p.get("id") != plan_id]
        if self.data.get("default_plan_id") == plan_id:
            self.data["default_plan_id"] = None
        self.save()
