...
class BridgeSystemImageTests(unittest.TestCase):
    ...
    def test_system_image_builds_native_wbadmin_command(self):
        with tempfile.TemporaryDirectory() as td:
            target={"name":"ImageDisk","path":td}
            completed=mock.Mock(returncode=0,stdout="OK",stderr="")
            with mock.patch.object(simage,"_is_admin",return_value=True), mock.patch("system_image_v180.subprocess.run",return_value=completed) as run:
