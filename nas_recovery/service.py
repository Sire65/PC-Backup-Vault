...
from .safety import DEFAULT_POLICY, RecoverySafetyPolicy, UnsafeRecoveryOperation
from .target_guard import image_target_disk_is_safe
...
""".strip()
        ...
        for row in rows:
            ...
            disks.append(PhysicalDisk(number=int(row.get("Number") or 0), model=str(row.get("FriendlyName") or ""), serial=str(row.get("SerialNumber") or "").strip(), bus=str(row.get("BusType") or ""), size=int(row.get("Size") or 0), status=str(status or ""), partition_style=str(row.get("PartitionStyle") or ""), is_offline=bool(row.get("IsOffline")), is_read_only=bool(row.get("IsReadOnly"))))
...
""".strip()
    ...
    def smart_report(self, disk: PhysicalDisk) -> str:
        ...
        if not smartctl:
            candidates = [Path(r"C:\Program Files\smartmontools\bin\smartctl.exe"), Path(r"C:\Program Files (x86)\smartmontools\bin\smartctl.exe")]
        ...
        completed = subprocess.run([smartctl, "-a", device], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, errors="replace", timeout=60, creationflags=CREATE_NO_WINDOW)
    ...
    def read_test(self, disk: PhysicalDisk, amount: int = 4 * 1024 * 1024) -> ReadTestResult:
        ...
        with open(device, "rb", buffering=0) as handle:
            ...
            while remaining > 0:
                ...
                if not chunk:
                    break
                digest.update(chunk); done += len(chunk); remaining -= len(chunk)
        if done == 0:
            raise RuntimeError("Vom Datenträger konnten keine Daten gelesen werden.")
        return ReadTestResult(device, done, digest.hexdigest())

    def create_image(self, disk: PhysicalDisk, destination: str | Path, *, chunk_size: int = 4 * 1024 * 1024, progress: Callable[[int, int], None] | None = None, should_cancel: Callable[[], bool] | None = None) -> Path:
        """Copy one physical disk to an image file without writing to the source.

        Before opening the target, Windows resolves its backing disk read-only.
        A local image target is blocked unless it is proven to be a different
        physical disk. UNC/network targets are not the local source disk.
        """
        ...
        if os.name == "nt" and not str(target).startswith("\\\\"):
            resolution = WindowsPathDeviceResolver().resolve(target)
            if not resolution.known:
                raise UnsafeRecoveryOperation("Image-Zieldatenträger konnte nicht eindeutig bestimmt werden; Imaging wurde sicherheitshalber blockiert.")
            if not image_target_disk_is_safe(disk.number, resolution.disk_number):
                raise UnsafeRecoveryOperation("Image-Ziel liegt auf derselben physischen Platte wie die Quelle; Imaging wurde blockiert.")
        ...
        mode = "ab" if resume else "wb"; done = resume
