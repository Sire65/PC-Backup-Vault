...
class RecoveryAssistantWindow(tk.Toplevel):
    ...
    def __init__(self, parent, selected_disk=None):
        ...
        normalize_window_geometry(self, 1120, 760, 920, 620)
    ...
    def _build(self):
        ...
        ttk.Label(left, text="Image-first · Original nie reparieren · nur freigegebene Schritte sind anklickbar", style="Muted.TLabel").pack(anchor="w")
        ...
        actions = ttk.LabelFrame(body, text="Aktionen", padding=10); actions.pack(fill="x", pady=(10,0))
        self.btn_source = ttk.Button(actions, text="1. Ausgewählte Quelle übernehmen", command=self._take_source)
        ...
        self.btn_audit = ttk.Button(footer, text="Auditbericht speichern", command=self._save_audit)
        self.btn_audit.pack(side="left")
        self.ready_label = ttk.Label(footer, style="Section.TLabel"); self.ready_label.pack(side="right")
    ...
    def _confirm_assessment(self):
        if not messagebox.askyesno("Quellprüfung", "Wurden Datenträgerdetails/SMART bzw. Read-only-Lesetest geprüft?\n\nEs wird keine Reparatur ausgeführt.", parent=self):
            return
        try:
            self.coordinator.mark_source_assessed()
        except Exception as exc:
            messagebox.showerror("Recovery-Assistent", str(exc), parent=self)
        self._refresh()
    ...
    def _choose_image(self):
        ...
        try:
            self.coordinator.attach_completed_image(path, device_id=resolved.device_id)
        except Exception as exc:
            messagebox.showerror("Recovery-Assistent", str(exc), parent=self)
        self._refresh()
    ...
    def _verify(self):
        try:
            ...
            messagebox.showerror("Image-Verifikation", str(exc), parent=self)
        self._refresh()
    ...
    def _analyze(self):
        try:
            ...
            messagebox.showerror("Image-Analyse", str(exc), parent=self)
        self._refresh()
    ...
    def _choose_target(self):
        ...
        try:
            self.coordinator.select_recovery_target(path, device_id=resolved.device_id)
        except Exception as exc:
            messagebox.showerror("Rettungsziel", str(exc), parent=self)
        self._refresh()
    ...
    def _save_audit(self):
        ...
        if path:
            save_recovery_audit(self.coordinator.session, path)
    ...
    def _refresh(self):
        ...
        self.info.configure(text=f"Physische Geräte: Quelle {ids[0]} · Image-Ziel {ids[1]} · Rettungsziel {ids[2]}. Recovery wird nur bei drei eindeutig verschiedenen Geräten freigegeben.")
        self.ready_label.configure(text="RECOVERY FREIGEGEBEN" if state.allowed(RecoveryStage.RECOVER) else "Recovery gesperrt")
