...
def _with_storage_targets(store, payload: dict) -> dict:
    """Attach read-only target health without exposing paths or credentials."""
    out = dict(payload)
    try:
        out["storageTargets"] = collect_storage_health(store)
    except Exception:
        # Telemetry itself must keep working even if one target probe fails badly.
        out["storageTargets"] = [
            {"id":"nas_backup","name":"NAS Backup","kind":"nas","status":"unknown","latencyMs":None,"checkedAt":datetime.now(timezone.utc).isoformat(),"detail":"Zielstatus konnte nicht ermittelt werden"},
            {"id":"hidrive_1","name":"HiDrive 1","kind":"hidrive","status":"unknown","latencyMs":None,"checkedAt":datetime.now(timezone.utc).isoformat(),"detail":"Zielstatus konnte nicht ermittelt werden"},
            {"id":"hidrive_2","name":"HiDrive 2","kind":"hidrive","status":"unknown","latencyMs":None,"checkedAt":datetime.now(timezone.utc).isoformat(),"detail":"Zielstatus konnte nicht ermittelt werden"},
        ]
    return out
