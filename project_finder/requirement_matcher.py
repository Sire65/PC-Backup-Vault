...
def tokens(text: str) -> list[str]:
    ...
    words = re.findall(r"[a-zA-Z0-9äöüÄÖÜß]{3,}", normalized)
...
def match_requirement(requirement: str, evidence_texts: Iterable, *, min_distinct: int = 2) -> dict:
    ...
    for idx, row in enumerate(evidence):
        present = set(tokens(row["text"]))
        matched = [term for term in req if term in present]
        score = round(100 * len(matched) / max(len(req), 1))
        candidate = (score, matched, idx, row)
        if len(matched) > len(best[1]) or (len(matched) == len(best[1]) and score > best[0]):
            best = candidate
    score, matched, idx, row = best
    required = min(min_distinct, len(req))
    lexical_ok = len(matched) >= required and score >= 35
    kind = row["kind"] if row else "unknown"
    if lexical_ok and kind in STRONG_EVIDENCE_KINDS:
        state = "FOUND"
    elif lexical_ok and kind in WEAK_EVIDENCE_KINDS:
        state = "WEAK_ONLY"
    else:
        state = "MISSING"
...
def annotate_findings(findings: Iterable[dict], evidence_by_project: dict[str, Iterable]) -> list[dict]:
    ...
    for finding in findings:
        ...
        row["git_requirement_match_evidence_kind"] = result["evidence_kind"]
        row["git_requirement_match_evidence_ref"] = result["evidence_ref"]
        rows.append(row)
    return rows
