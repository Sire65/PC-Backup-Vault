...
class DevelopmentItem:
    ...
    local_git_relation: str = "NOT_CHECKED"
    build_evidence: str = "NOT_CHECKED"
...
def summarize(items: Iterable[DevelopmentItem]) -> dict:
    ...
    return {
        "schema": "pc-backup-vault.development-center.v3",
        "counts": counts,
        "items": [asdict(x) for x in rows],
        "rules": {"chat_is_proof": False, "green_requires_test_pass": True, "local_newer_blocks_green": True, "diverged_blocks_green": True, "idea_missing_is_not_lost_development": True},
    }
