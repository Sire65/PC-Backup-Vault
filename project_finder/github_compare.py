...
from project_finder.git_handoff import guess_repo
...
def compare_item(item: ScanItem, repo_index: dict[str, dict], repo: str, *, token: str = '', verify_content: bool = True) -> dict:
    ...
    if remote is None:
        return {'path': str(local), 'repo': repo, 'repo_path': repo_path, 'state': 'LOCAL_ONLY', 'local_sha256': local_hash, 'remote_sha256': '', 'reason': 'Pfad im GitHub-Repository nicht vorhanden'}
    remote_hash = ''
    ...
    return {'path': str(local), 'repo': repo, 'repo_path': repo_path, 'state': state, 'local_sha256': local_hash, 'remote_sha256': remote_hash, 'reason': reason}
...
def compare_inventory(items: Iterable[ScanItem], *, token: str | None = None, verify_content: bool = True) -> dict:
    ...
    for item in items:
        ...
        if not repo:
            unassigned.append({'path': item.path, 'state': 'UNASSIGNED', 'reason': 'Kein Ziel-Repository sicher erkannt'})
            continue
    ...
    for repo, repo_items in grouped.items():
        try:
            ...
            for item in repo_items:
                results.append({'path': item.path, 'repo': repo, 'repo_path': candidate_repo_path(Path(item.path), repo), 'state': 'REPO_UNAVAILABLE', 'reason': str(exc), 'local_sha256': item.sha256, 'remote_sha256': ''})
            continue
        for item in repo_items:
    ...
    return {'schema': 'pc-backup-vault.github-compare.v1', 'read_only': True, 'counts': counts, 'repo_errors': repo_errors, 'items': results}
