...
class GitHubCompareTests(unittest.TestCase):
    ...
    def test_secret_and_local_only_inventory_items_are_excluded_before_api_access(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp) / 'DP2'
            root.mkdir()
            source = root / 'app.py'
            source.write_text('print(42)', encoding='utf-8')
            secret = root / '.env'
            secret.write_text('SUPABASE_SERVICE_ROLE_KEY=very-secret-value', encoding='utf-8')
            local_doc = root / 'notes.bin'
            local_doc.write_bytes(b'local')
            items = scan([root], hash_only_interesting=False, include_hidden=True)
            with patch('project_finder.github_compare.fetch_repo_tree', return_value={}) as fetch_tree:
                report = compare_inventory(items, token='test', verify_content=False)
            self.assertEqual(report['candidate_count'], 1)
            self.assertGreaterEqual(report['excluded_count'], 2)
            self.assertNotIn(str(secret), [row.get('path') for row in report['items']])
            fetch_tree.assert_called_once_with('Sire65/Dienstplan', token='test')
