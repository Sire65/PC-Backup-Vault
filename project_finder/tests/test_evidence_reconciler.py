...
class EvidenceReconcilerTests(unittest.TestCase):
    ...
    def test_explicit_requirement_match_plus_pass_is_green(self):
        findings = [{"project": "Dienstplan", "kind": "IMPLEMENTATION_CLAIM", "text": "Fehleranalyse ist fertig", "git_requirement_match": "FOUND", "message_id": "m1"}]
        repos = [{"project": "DP2", "repository": "Sire65/Dienstplan", "head_sha": "abc", "latest_test": "PASS", "local_state": "SAME", "future_field": "ignored"}]
        result = reconcile_evidence(findings, repos)
        self.assertEqual(result["items"][0]["status"], "GREEN")
        self.assertEqual(result["items"][0]["project"], "DP2 / Dienstplan")
        self.assertEqual(result["items"][0]["source_message_id"], "m1")
