...
class ProjectFinderTab(ttk.Frame):
    """Embeddable ttk.Frame for an existing ttk.Notebook.

    No existing backup behavior is modified. The host application only needs to add this
    frame as a new notebook tab when ready.
    """
    ...
    def _build(self):
        ...
        results = ttk.LabelFrame(self, text='3 · Ergebnisse und Löschvorschläge'); results.pack(fill=BOTH, expand=True, padx=12, pady=6)
    ...
    def start_scan(self):
        ...
        self.status_var.set('Analyse läuft · Nur Lesen')
    ...
    def _scan_worker(self):
        try:
            ...
            self.after(0,lambda:self.status_var.set('Fehler bei Analyse'))
    ...
    def _render(self):
        ...
        for i in self.items:
            ...
            self.tree.insert('',END,values=(lamp,i.name,i.version_hint,i.category,self._fmt_size(i.size),i.modified_iso,action,i.path))
            shown+=1
    ...
    def quarantine_selected(self):
        ...
        if not messagebox.askyesno('Sichere Bereinigung',f'{len(paths)} markierte Dateien werden NICHT endgültig gelöscht, sondern in eine wiederherstellbare Quarantäne verschoben. Fortfahren?'):
            return
        manifest=quarantine(paths,self.quarantine_root)
        messagebox.showinfo('Sichere Bereinigung',f'{len(manifest)} Dateien wurden in Quarantäne verschoben.\nManifest zur Wiederherstellung wurde gespeichert.')
        self.status_var.set(f'Quarantäne: {len(manifest)} Dateien verschoben')

    def export(self,kind:str):
        if not self.items:
            messagebox.showinfo('Projekt-Finder','Noch keine Analyse vorhanden.'); return
        ...
        if not p:return
        (export_json if kind=='json' else export_csv)(self.items,p)
        self.status_var.set(f'Export gespeichert: {p}')

    def copy_summary(self):
        if not self.items:return
        proposals=cleanup_candidates(self.items)
        dups=sum(1 for x in proposals if x['proposed_action']=='QUARANTINE')
        reviews=sum(1 for x in proposals if x['proposed_action']=='REVIEW')
        total=sum(x['size'] for x in proposals)
        reclaim=sum(x['size'] for x in proposals if x['proposed_action']=='QUARANTINE')
        text=(f'PC Backup Vault Projekt-Finder\nDateien: {len(proposals):,}\nGesamtgröße erfasst: {self._fmt_size(total)}\n'
              f'Sichere Dubletten-Kandidaten: {dups:,}\nWeitere Prüfkandidaten: {reviews:,}\n'
              f'Potentiell sofort rückgewinnbar durch Dubletten-Quarantäne: {self._fmt_size(reclaim)}')
